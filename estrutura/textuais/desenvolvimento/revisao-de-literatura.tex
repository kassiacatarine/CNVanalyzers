...
O genoma é constituído de regiões codificantes e não codificantes, elas denominadas como éxons e íntrons respectivamente. As regiões codificantes constituem-se de sequências de proteínas, elas representam cerca de 1\% do genoma \cite{Ng2009}.Na \autoref{fig:figura-exoma} é representado essas regiões presentes na estrutura do DNA e RNA, podendo identificar os éxons como as áreas destacadas com uma camada colorida e os íntrons em cada espaço existente entre as sequências dos éxons.

\begin{figure}[!htb]
    \centering
    \caption{Exemplificação das regiões codificantes de proteínas (éxons ou exoma) na estrutura do DNA e RNA}
    \includegraphics[width=0.8\textwidth]{./dados/figuras/exoma}
    \fonte{Autoria Própria}
    \label{fig:figura-exoma}
\end{figure}

A quantidade de 1\% do genoma pode chegar a conter aproximadamente 180 mil éxons, esse conjunto de regiões codificantes de proteínas é conhecido como exoma, uma representação dele pode ser vista na \autoref{fig:figura-exoma} \cite{Bamshad2011}. O mesmo pode ser obtido pelo processo de sequenciamento completo do exoma, ele correspondente a separação dos éxons contidos no genoma, o seu uso vem sendo registrado em demasiados diagnósticos moleculares de detecção de distúrbios genéticos, por ser possível identificar variantes relacionadas a doenças com as informações geradas \cite{Bamshad2011}\cite{Lee2014}\cite{HutchisonIII2007}. 

Apesar da aplicação do sequenciamento para obtenção do exoma através da extração dos éxons, ainda existe dúvidas sobre quais regiões são exatamente codificantes de proteínas, assim não podendo ser especificado qual o exato conjunto equivalente ao exoma \cite{Bamshad2011}.
...
A partir dessa realidade surgiu vários métodos baseados no \textit{next-genaration sequencing} (NGS), esses métodos são um conjunto de planos executados em conjunto para obter uma coleção maior de dados com um custo reduzido e melhor performance \cite{Metzker2010}\cite{Zhao2013}, como visto nos resultados produzidos pelos sequenciadores do Solexa \cite{Bennett2004}, HiSeq 2000, SOLID e outros \cite{Edwards2011} \cite{Linnarsson2010}. Esses equipamentos estão possibilitando uma maior cobertura e resolução, assim sendo possível identificar mais variações no número de cópias do genoma \cite{Zhao2013}. 
...
A \textit{copy number variation} (CNV) ou no português variação no número de cópias, são alterações que ocorrem na região genômica, elas se classificam como inserções, duplicações e deleções na estrutura do cromossomo, como representado na \autoref{fig:figura-copy-number-variation} \cite{Zhao2013}\cite{Redon2006}. Essas variações podem ser encontradas em todos os seres humanos, podendo não representar nenhuma alteração ou ser responsável por doenças de origem mendeliana, sua existência e descoberta são essenciais para o entendimento da composição genômica \cite{Redon2006} \cite{Feuk2006}.
...
\[H_{1} : F_{1} = ... = F_{k_{1}}\neq F_{k_{1}}_{+}_{1} = ... = F_{k_{2}}\neq F_{k_{2}}_{+}_{1} = ... F_{k_{q}}\neq F_{k_{q}}_{+}_{1} ... = F_{n},\]
...
\[H_1 : \theta_{1} = ... = \theta_{k_{1}} \not = \theta_{k_{1}}_{+}_{1} = ... = \theta_{2} \not = ... \not = \theta_{k_{q}}_{-}_{1} = ... = \theta_{k_{q}} \not = \theta_{k_{q}}_{+}_{1}... = \theta_{n},\]
...
% A utilização desses algoritmos focados na detecção do ponto de mudança em CNVs é bastante explorada como visto em (referencia)(referencia), a seguir encontra-se a definição de alguns dos algoritmos mais utilizados no propósito citado acima:
...
\input{./dados/algoritmos/OP} 
\input{./dados/algoritmos/PELT} 
...
O funcionamento do algoritmo é determinado pelos passos de determinação de uma matriz de similaridade entre pontos $x$ e $y$, assim obtendo $S_{i}_{j} = (x_{i}, y_{j})$.A partir do calculo da matriz $S$ com a medida de similaridade Gaussiana é obtido uma matriz de afinidade, usada para o repartimento entre si obtendo \textit{clusters}, assim podendo ser identificados os pontos de mudança \cite{Uzai2019}. 
